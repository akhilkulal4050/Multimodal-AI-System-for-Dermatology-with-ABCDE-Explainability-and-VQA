...
from typing import Any, List, Optional, Tuple, Union, Dict, Set, Callable
from transformers.models.mistral.modeling_mistral import (Callable, Optional, torch, nn, ACT2FN, Cache, use_kernel_func_from_hub, FlashAttentionKwargs, ROPE_INIT_FUNCTIONS, dynamic_rope_update, ALL_ATTENTION_FUNCTIONS, Unpack, TransformersKwargs, maybe_autocast, MistralConfig)
...
class MistralMLP(nn.Module):
    ...
    def forward(self, x):
        return MistralMLP_forward(self, x=x)
...
class MistralAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        return MistralAttention_forward(self, hidden_states=hidden_states, position_embeddings=position_embeddings, attention_mask=attention_mask, past_key_values=past_key_values, cache_position=cache_position, **kwargs)


@torch.compile(fullgraph = True, dynamic = True, options = torch_compile_options)
def MistralRMSNorm_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class MistralRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        return MistralRMSNorm_forward(self, hidden_states=hidden_states)
...
class MistralRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        return MistralRotaryEmbedding_forward(self, x=x, position_ids=position_ids)
